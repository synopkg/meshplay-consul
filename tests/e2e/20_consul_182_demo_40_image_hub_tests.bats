...
EOT
  ...
  run bash -c "echo '$INSTALL_IMAGE_HUB' | grpcurl --plaintext -d @ $MESHPLAY_ADAPTER_ADDR:10002 meshes.MeshService.ApplyOperation"
  [ "$status" -eq 0 ]
}
...
EOT
  ...
  run bash -c "echo '$DELETE_IMAGE_HUB' | grpcurl --plaintext -d @ $MESHPLAY_ADAPTER_ADDR:10002 meshes.MeshService.ApplyOperation"
  [ "$status" -eq 0 ]
}
